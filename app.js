...
function executeTrade() {
    ...
    const moveItems = (from, to, indices) => {
        indices.sort((a,b) => b-a).forEach(idx => to.inventory.push(from.inventory.splice(idx, 1)[0]));
    };
    moveItems(p1, p2, t.offer1.items); moveItems(p2, p1, t.offer2.items);

    alert("¡Intercambio cerrado con éxito!");
    activeTrade = null;
...
function changeRole() {
    ...
    saveAll(); applyRoleUI(); renderNavigation(); renderParty();
}
...
function renderNavigation() {
    ...
    nav.innerHTML = `
        ...
        ${isM ? `<button onclick="window.showTab(\'tab-combat\')"><i class="fa-solid fa-bolt"></i><span>Batalla</span></button><button onclick="window.showTab(\'tab-loot\')"><i class="fa-solid fa-coins"></i><span>Oro</span></button>` : \'\'}
...
function generateRandomEncounter() { alert("Simulación: Monstruo CR 1"); }
function generateLoot() { alert("Simulación: 50 gp hallados"); }
