...
function renderCampaignList() {
    ...
    if (campaigns.length === 0) {
        container.innerHTML = '<p style="color:#666; font-style:italic; padding:20px; text-align:center;">No hay campañas creadas.</p>';
        return;
    }
    container.innerHTML = campaigns.map(c => `
        <div class="campaign-item">
            ...
            <button onclick="deleteCampaign(\'' + c.id + '\')" style="background:transparent; border:none; color:#ff4444;"><i class="fa-solid fa-trash"></i></button>
        </div>
    `).join('');
}
...
function saveCampaign() {
    ...
    if (!name) return;
    const newCamp = { id: 'camp_' + Date.now(), name: name, party: [], notices: [] };
...
function deleteCampaign(id) {
    if (confirm("¿Borrar esta campaña?")) {
...
function selectCampaign(id) {
    ...
    if (!camp) { currentCampaignId = null; saveAll(); renderCampaignList(); return; }
    currentCampaignId = id; party = camp.party || [];
...
function renderTradeUI() {
    ...
    if (!t.p1Confirmed) selectorArea.innerHTML = `<select onchange="activeTrade.p2Id=this.value; renderTradeUI();">${party.filter(p=>p.id!=t.p1Id).map(p=>`<option value="${p.id}" ${p.id==t.p2Id?'selected':''}>${p.name}</option>`).join('')}</select>`;
    else selectorArea.innerHTML = '';
...
function confirmTradePart(num) {
    ...
    if ((num==1?t.offer1.gold:t.offer2.gold) > char.wallet.gp + (char.wallet.gp_fake || 0)) return alert("Oro insuficiente.");
...
function executeTrade() {
    ...
    const transfer = (from, to, amount) => {
        ...
        const fRatio = (from.wallet.gp_fake || 0) / total; const fGive = Math.floor(amount * fRatio); const rGive = amount - fGive;
        from.wallet.gp -= rGive; from.wallet.gp_fake -= fGive; to.wallet.gp += rGive; to.wallet.gp_fake += fGive;
    };
    transfer(p1, p2, t.offer1.gold); transfer(p2, p1, t.offer2.gold);
    const moveI = (from, to, idxs) => { idxs.sort((a,b) => b-a).forEach(i => to.inventory.push(from.inventory.splice(i, 1)[0])); };
        ...
        <div class="char-card">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div onclick="toggleInventory(${c.id})" style="flex:1; cursor:pointer;">
                    ...
                    <p style="margin:0; font-size:0.8rem;">💰 ${c.wallet.gp + (c.wallet.gp_fake||0)} gp</p>
                </div>
                <button onclick="openTrade(${c.id})" class="btn-mini">INTERCAMBIO</button>
            </div>
            <div id="inv-${c.id}" style="display:none; background:#0a0a0a; padding:10px; margin-top:10px;">
                ${c.inventory.map(it=>`<p>• ${it.name}</p>`).join('') || '<p>Vacio</p>'}
                ${(currentRole=='master' && c.wallet.gp_fake > 0) ? `<p style="color:red;">⚠️ ORO FALSO: ${c.wallet.gp_fake}</p>`:''}
            </div>
        </div>
    `).join('');
}
...
function generateRandomEncounter() { alert("Buscando monstruos..."); }
