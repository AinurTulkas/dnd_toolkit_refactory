...
function createCharacter() {
    ...
    if(classData && classData.starting_items) {
        classData.starting_items.forEach(itemName => {
            const itemMatch = srdData.items.find(i => i.name.toLowerCase().trim() === itemName.toLowerCase().trim());
            inventory.push(itemMatch ? {...itemMatch} : {name: itemName, category: "Equipo", weight: 1});
        });
    }
...
function generateLoot() {
    ...
    const distribution = { cp: Math.floor(Math.random() * 15 * coinFactor), sp: Math.floor(Math.random() * 8 * coinFactor), gp: Math.floor(Math.random() * 1.5 * coinFactor) };
    const lootItems = [];
    if(Math.random() < 0.2) lootItems.push({...srdData.items[Math.floor(Math.random() * srdData.items.length)]});
    ...
    document.getElementById('loot-result').innerHTML = `
        <div class="card" style="background:#1a1a1a; border:1px dashed var(--gold); padding:20px; text-align:left;">
            <h3 style="font-family:'Cinzel'; color:var(--gold); margin:0 0 15px 0;">Botín Realista</h3>
            ${Object.entries(distribution).map(([type, amount]) => `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #333; padding:8px 0;"><span style="font-weight:bold; color:${getCoinColor(type)}">${amount} ${type}</span><div style="display:flex; gap:5px;"><input type="number" id="split-${type}" value="${amount}" min="1" max="${amount}" style="width:70px; padding:4px;"><button onclick="assignCoins('${type}')" style="background:var(--gold); border:none; border-radius:4px; padding:4px 8px; font-weight:bold; cursor:pointer;">DAR</button></div></div>`).join('')}
            <div style="margin-top:15px;">${lootItems.map((item, idx) => `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #333; padding:8px 0;"><span>• ${item.name}</span><button onclick="showAssignItem(${idx})" style="background:#333; color:var(--gold); border:1px solid var(--gold); border-radius:4px; padding:4px 8px; cursor:pointer;">ASIGNAR</button></div><div id="assign-item-${idx}" style="display:none; margin-top:5px; background:#222; padding:10px;"></div>`).join('')}</div>
        </div>
    `;
}
...
function assignCoins(type) {
    const amount = parseInt(document.getElementById(`split-${type}`).value); if (amount <= 0 || isNaN(amount)) return;
    ...
    container.style.display = 'block'; container.innerHTML = `<div class="card" style="padding:25px; background:#111; border:1px solid var(--gold);"><p style="color:var(--gold); margin-bottom:15px;">Dar ${amount}${type} a:</p>${charList}</div>`;
}
function confirmAssignCoins(type, amount, charId) { const char = party.find(c => c.id == charId); if (char) { char.wallet[type] += amount; saveAll(); } document.getElementById('assign-area-global').style.display = 'none'; }
function showAssignItem(idx) { const area = document.getElementById(`assign-item-${idx}`); area.style.display = 'block'; area.innerHTML = party.map(c => `<button onclick="confirmAssignItem(${idx}, '${c.id}')" style="display:block; width:100%; margin-bottom:2px; background:#111; color:#fff; border:1px solid #444; padding:10px; cursor:pointer;">→ ${c.name}</button>`).join(''); }
function confirmAssignItem(idx, charId) { const char = party.find(c => c.id == charId); if (char) { char.inventory.push({...currentLoot.items[idx]}); saveAll(); } document.getElementById(`assign-item-${idx}`).parentElement.style.display = 'none'; }
...
function renderInventory(id) {
    ...
    container.innerHTML = `<div style="margin-bottom:15px; padding:15px; background:#000; border-radius:8px; border:1px solid #333;"><span style="color:var(--gold)">PESO: ${w.toFixed(1)} / ${cap} lb</span></div>` + 
        ...
        char.inventory.map(item => `<div style="background:#111; padding:12px; border-radius:8px; margin-bottom:8px; border:1px solid #333;">${item.name} (${item.weight} lb)</div>`).join('');
}
