...
function renderTradeUI() {
    ...
    if (!t.p1Confirmed) sel.innerHTML = `<select onchange="activeTrade.p2Id=this.value; window.renderTradeUI();" style="background:#222; color:var(--gold); border:1px solid #444;">${party.filter(p=>p.id!=t.p1Id).map(p=>`<option value="${p.id}" ${p.id==t.p2Id?\'selected\':\'\'}>${p.name}</option>`).join(\'\')}</select>`;
    else sel.innerHTML = \'\';
...
function executeTrade() {
    ...
    const moveGold = (from, to, amount) => {
        ...
        const ratio = (from.wallet.gp_fake || 0) / total; const fGive = Math.floor(amount * ratio); const rGive = amount - fGive;
    ...
    alert("¡Intercambio cerrado con éxito!"); activeTrade = null; const camp = campaigns.find(c => c.id === currentCampaignId); if (camp) camp.party = party; saveAll(); closeTrade(); renderParty();
}
...
function renderParty() {
    ...
    container.innerHTML = party.map(c => `<div class="char-card" style="display:flex; flex-direction:column; background:#111; border:1px solid #222; margin-bottom:10px; padding:15px; border-radius:8px;"><div style="display:flex; justify-content:space-between; align-items:center;"><div onclick="window.toggleInventory(${c.id})" style="flex:1; cursor:pointer;"><h3 style="margin:0; color:var(--gold);">${c.name}</h3><p style="margin:0; font-size:0.8rem; color:#888;">💰 ${c.wallet.gp + (c.wallet.gp_fake||0)} gp</p></div><button onclick="window.openTrade(${c.id})" class="btn-mini" style="border:1px solid var(--gold); color:var(--gold); background:none; padding:6px 12px; border-radius:6px; font-weight:bold; cursor:pointer;">INTERCAMBIO</button></div><div id="inv-${c.id}" style="display:none; background:#0a0a0a; padding:10px; margin-top:10px; border-radius:6px;">${c.inventory.map(it=>`<p style="margin:4px 0; font-size:0.8rem;">• ${it.name}</p>`).join(\'\') || \'<p style="color:#444;">Vacío</p>\'}${ (currentRole==\'master\' && c.wallet.gp_fake > 0) ? `<p style="color:red; font-size:0.7rem; margin-top:5px;">⚠️ ORO FALSO: ${c.wallet.gp_fake}</p>` : \'\' }</div></div>`).join(\'\');
}
...
function createCharacter() {
    const name = document.getElementById(\'char-name\').value.trim(); if (!name) return alert("Escribe un nombre."); const cls = document.getElementById(\'char-class\').value; const race = document.getElementById(\'char-race\').value;
    ...
    party.push(newChar); const camp = campaigns.find(c => c.id === currentCampaignId); if (camp) camp.party = party; saveAll(); renderParty(); document.getElementById(\'char-name\').value = \'\';
}
function generateRandomEncounter() { alert("Buscando enemigo..."); } function generateLoot() { alert("Generando loot..."); }
